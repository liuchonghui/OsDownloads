...
public class DownloadService extends Service {
    /**
     * The Service's view of the list of downloads. This is kept independently from the content
     * provider, and the Service only initiates downloads based on this data, so that it can deal
     * with situation where the data in the content provider changes or disappears.
     */
    ...
    private static int DEFAULT_THREAD_NUMBER = 5;
    ...
    private static ExecutorService buildDownloadExecutor() {
        ThreadPoolExecutor executor = (ThreadPoolExecutor) Executors.newFixedThreadPool(DEFAULT_THREAD_NUMBER);
        return executor;
    }
    ...
    public void onStart(Intent intent, int startId) {
        ...
        if (Constants.LOGV) {
            ...
            Log.i("Info", "update from provider");
        }
        updateFromProvider();
    }
...
//                                Intent intent = new Intent(Constants.ACTION_WAKEUP);
//                                intent.setClassName("tv.zhenjing.vitamin.downloads", DownloadReceiver.class.getName());
...
//                                        PendingIntent.getBroadcast(DownloadService.this, 0, intent, PendingIntent.FLAG_ONE_SHOT));
                ...
                boolean networkAvailable = InfoHelpers.isNetworkAvailable(DownloadService.this);
                boolean networkRoaming = InfoHelpers.isNetworkRoaming(DownloadService.this);
                long now = System.currentTimeMillis();

                Cursor cursor = getContentResolver().query(Downloads.CONTENT_URI, null, null, null, Downloads._ID);
                if (cursor == null) {
                    // TODO: this doesn't look right, it'd leave the loop in an inconsistent state
                    if (Constants.LOGV) {
                        Log.v(Constants.TAG, "Cursor is null");
                    }
                    return;
                }
                cursor.moveToFirst();
                ...
                while (!isAfterLast || arrayPos < mDownloads.size()) {
                    if (isAfterLast) {
                        ...
                        if (Constants.LOGV) {
                            ...
                            Log.v(Constants.TAG, "Array update: trimming " + arrayId + " @ " + arrayPos);
                        }
                        /*
                         * if (shouldScanFile(arrayPos) && mediaScannerConnected()) { scanFile(null,
                         * arrayPos); }
                         */
                        ...
                        if (arrayPos == mDownloads.size()) {
                            ...
                            if (arrayId < id) {
                                ...
                                if (Constants.LOGV) {
                                    Log.v(Constants.TAG, "Array update: removing " + arrayId + " @ " + arrayPos);
                                }
                                deleteDownload(arrayPos); // this advances in the array
                            } else if (arrayId == id) {
                                ...
                                if (Constants.LOGV) {
                                    Log.v(Constants.TAG, "Array update: inserting " + id + " @ " + arrayPos);
                                }
                                insertDownload(cursor, arrayPos, networkAvailable, networkRoaming, now);
                                if (visibleNotification(arrayPos)) {
                                    keepService = true;
    ...
    private void updateDownload(Cursor cursor, int arrayPos, boolean networkAvailable, boolean networkRoaming, long now) {
        ...
        info.mNoIntegrity = cursor.getInt(cursor.getColumnIndexOrThrow(Downloads.COLUMN_NO_INTEGRITY)) == 1;
        ...
        info.mDestination = cursor.getInt(cursor.getColumnIndexOrThrow(Downloads.COLUMN_DESTINATION));
        int newVisibility = cursor.getInt(cursor.getColumnIndexOrThrow(Downloads.COLUMN_VISIBILITY));
        if (info.mVisibility == Downloads.VISIBILITY_VISIBLE_NOTIFY_COMPLETED && newVisibility != Downloads.VISIBILITY_VISIBLE_NOTIFY_COMPLETED
        ...
        info.mLastMod = cursor.getLong(cursor.getColumnIndexOrThrow(Downloads.COLUMN_LAST_MODIFICATION));
        info.mPackage = stringFromCursor(info.mPackage, cursor, Downloads.COLUMN_NOTIFICATION_PACKAGE);
        ...
        info.mTotalBytes = cursor.getInt(cursor.getColumnIndexOrThrow(Downloads.COLUMN_TOTAL_BYTES));
        info.mCurrentBytes = cursor.getInt(cursor.getColumnIndexOrThrow(Downloads.COLUMN_CURRENT_BYTES));
        ...
        if (info.canUseNetwork(networkAvailable, networkRoaming)) {
            if (info.isReadyToRestart(now)) {
                if (Constants.LOGV) {
                    Log.v(Constants.TAG, "Service spawning thread to handle updated download " + info.mId);
                }
                if (info.mHasActiveThread) {
//					throw new IllegalStateException("Multiple threads on same download on update");
                    if (Constants.LOGV) {
                        Log.v(Constants.TAG, "Multiple threads on same download on update ");
                    }
                    return;
                }
                ...
                getContentResolver().update(ContentUris.withAppendedId(Downloads.CONTENT_URI, info.mId), values, null, null);
    ...
    private void insertDownload(Cursor cursor, int arrayPos, boolean networkAvailable, boolean networkRoaming, long now) {
        ...
        int failedColumn = cursor.getColumnIndexOrThrow(Constants.FAILED_CONNECTIONS);
        int retryRedirect = cursor.getInt(cursor.getColumnIndexOrThrow(Constants.RETRY_AFTER_X_REDIRECT_COUNT));
        DownloadInfo info = new DownloadInfo(cursor.getInt(cursor.getColumnIndexOrThrow(Downloads._ID)), cursor.getString(cursor
                .getColumnIndexOrThrow(Downloads.COLUMN_URI)), cursor.getInt(cursor.getColumnIndexOrThrow(Downloads.COLUMN_NO_INTEGRITY)) == 1,
                cursor.getString(cursor.getColumnIndexOrThrow(Downloads.COLUMN_FILE_NAME_HINT)), cursor.getString(cursor
                .getColumnIndexOrThrow(Downloads._DATA)), cursor.getString(cursor.getColumnIndexOrThrow(Downloads.COLUMN_MIME_TYPE)),
                cursor.getInt(cursor.getColumnIndexOrThrow(Downloads.COLUMN_DESTINATION)), cursor.getInt(cursor
                .getColumnIndexOrThrow(Downloads.COLUMN_VISIBILITY)), cursor.getInt(cursor.getColumnIndexOrThrow(Downloads.COLUMN_CONTROL)),
                cursor.getInt(statusColumn), cursor.getInt(failedColumn), retryRedirect & 0xfffffff, retryRedirect >> 28, cursor.getLong(cursor
                .getColumnIndexOrThrow(Downloads.COLUMN_LAST_MODIFICATION)), cursor.getString(cursor
                .getColumnIndexOrThrow(Downloads.COLUMN_NOTIFICATION_PACKAGE)), cursor.getString(cursor
                .getColumnIndexOrThrow(Downloads.COLUMN_NOTIFICATION_CLASS)), cursor.getString(cursor
                .getColumnIndexOrThrow(Downloads.COLUMN_NOTIFICATION_EXTRAS)), cursor.getString(cursor
                .getColumnIndexOrThrow(Downloads.COLUMN_COOKIE_DATA)), cursor.getString(cursor
                .getColumnIndexOrThrow(Downloads.COLUMN_USER_AGENT)),
                cursor.getString(cursor.getColumnIndexOrThrow(Downloads.COLUMN_REFERER)), cursor.getInt(cursor
                .getColumnIndexOrThrow(Downloads.COLUMN_TOTAL_BYTES)), cursor.getInt(cursor
                .getColumnIndexOrThrow(Downloads.COLUMN_CURRENT_BYTES)), cursor.getString(cursor.getColumnIndexOrThrow(Constants.ETAG)),
        ...
        if (info.mStatus == 0
                && (info.mDestination == Downloads.DESTINATION_EXTERNAL || info.mDestination == Downloads.DESTINATION_CACHE_PARTITION_PURGEABLE)
                && info.mMimeType != null && !Constants.MIMETYPE_DRM_MESSAGE.equalsIgnoreCase(info.mMimeType)) {
        ...
        if (info.canUseNetwork(networkAvailable, networkRoaming)) {
            if (info.isReadyToStart(now)) {
                if (Constants.LOGV) {
                    Log.v(Constants.TAG, "Service spawning thread to handle new download " + info.mId);
                }
                if (info.mHasActiveThread) {
//					throw new IllegalStateException("Multiple threads on same download on insert");
                    if (Constants.LOGV) {
                        Log.v(Constants.TAG, "Multiple threads on same download on insert");
                    }
                    return;
                }
                if (info.mStatus != Downloads.STATUS_RUNNING) {
            ...
            if (info.mStatus == 0 || info.mStatus == Downloads.STATUS_PENDING || info.mStatus == Downloads.STATUS_RUNNING) {
    ...
    private void removeSpuriousFiles() {
        ...
        Cursor cursor = null;
        try {
            cursor = getContentResolver().query(Downloads.CONTENT_URI, new String[]{Downloads._DATA}, null, null, null);
            if (cursor != null) {
    ...
    private void trimDatabase() {
        Cursor cursor = getContentResolver().query(Downloads.CONTENT_URI, new String[]{Downloads._ID}, Downloads.COLUMN_STATUS + " >= '200'",
                null, Downloads.COLUMN_LAST_MODIFICATION);
        if (cursor == null) {
        ...
        if (cursor.moveToFirst()) {
            ...
            while (numDelete > 0) {
                getContentResolver().delete(ContentUris.withAppendedId(Downloads.CONTENT_URI, cursor.getLong(columnId)), null, null);
    ...
    /**
     * Returns the amount of time (as measured from the "now" parameter) at which a download will be
     * active. 0 = immediately - service should stick around to handle this download. -1 = never -
     * service can go away without ever waking up. positive value - service must wake up in the
     * future, as specified in ms from "now"
     */
