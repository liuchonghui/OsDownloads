...
//            wakeLock.acquire();
            ...
            if (filename != null) {
                if (!InfoHelpers.isFilenameValid(filename, mContext)) {
                ...
                if (f.exists()) {
                    ...
                    /*
                     * else if (mInfo.mETag == null && !mInfo.mNoIntegrity) { // Tough luck, that's
                     * not a resumable download f.delete(); finalStatus =
                     * Downloads.STATUS_PRECONDITION_FAILED; notifyDownloadCompleted( finalStatus,
                     * false, 0, 0, false, filename, null, mInfo.mMimeType); return; }
                     */
                    else {
            ...
            if (stream != null && mInfo.mDestination == Downloads.DESTINATION_EXTERNAL && !Constants.MIMETYPE_DRM_MESSAGE.equalsIgnoreCase(mimeType)) {
                try {
                    stream.close();
                    stream = null;
                } catch (IOException ex) {
                    if (Constants.LOGV) {
                        Log.v(TAG, "exception when closing the file before download : " + ex);
                    }
                    // nothing can really be done if the file can't be closed
                }
            }
            /*
             * This loop is run once for every individual HTTP request that gets sent. The very
             * first HTTP request is a "virgin" request, while every subsequent request is done with
             * the original ETag and a byte-range.
             */
            http_request_loop:
            while (true) {
                ...
                try {
                    ...
                    if (Constants.LOGV) {
                        Log.d(TAG, "Arg exception trying to execute request for " + mInfo.mUri + " : " + ex);
                    }
                    ...
                    if (Constants.LOGV) {
                        if (InfoHelpers.isNetworkAvailable(mContext)) {
                            Log.i(TAG, "Execute Failed " + mInfo.mId + ", Net Up");
                        } else {
                            Log.i(TAG, "Execute Failed " + mInfo.mId + ", Net Down");
                        }
                    }
                    if (!InfoHelpers.isNetworkAvailable(mContext)) {
                        ...
                        if (Constants.LOGV) {
                            Log.d(TAG, "IOException trying to execute request for " + mInfo.mUri + " : " + ex);
                        }
                    ...
                    if (header != null) {
                        try {
                            ...
                            if (retryAfter < 0) {
                                ...
                                retryAfter += InfoHelpers.sRandom.nextInt(Constants.MIN_RETRY_AFTER + 1);
                                retryAfter *= 1000;
                            }
                ...
                if (statusCode == 301 || statusCode == 302 || statusCode == 303 || statusCode == 307) {
                    ...
                    if (redirectCount >= Constants.MAX_REDIRECTS) {
                        if (Constants.LOGV) {
                            Log.d(TAG, "too many redirects for download " + mInfo.mId + " at " + mInfo.mUri);
                        }
                    ...
                    if (header != null) {
                        if (Constants.LOGV) {
                            Log.v(TAG, "Location :" + header.getValue());
                        }
                        try {
                            newUri = new URI(mInfo.mUri).resolve(new URI(header.getValue())).toString();
                        } catch (URISyntaxException ex) {
                            if (Constants.LOGV) {
                                Log.d(TAG, "Couldn't resolve redirect URI " + header.getValue() + " for " + mInfo.mUri);
                            }
                            finalStatus = Downloads.STATUS_BAD_REQUEST;
                            request.abort();
                            break http_request_loop;
                ...
                if ((!continuingDownload && statusCode != Downloads.STATUS_SUCCESS) || (continuingDownload && statusCode != 206)) {
                    if (Constants.LOGV) {
                        Log.d(TAG, "http error " + statusCode + " for " + mInfo.mUri);
                    }
                    ...
                    if (!continuingDownload) {
                        ...
                        DownloadFileInfo fileInfo = InfoHelpers.generateSaveFile(mContext, mInfo.mUri, mInfo.mHint, headerContentDisposition,
                                headerContentLocation, mimeType, mInfo.mDestination,
                                (headerContentLength != null) ? Integer.parseInt(headerContentLength) : 0);
                        if (fileInfo.mStream == null) {
                        ...
                        if (Constants.LOGV) {
                            Log.v(TAG, "writing " + mInfo.mUri + " to " + filename);
                        }
                        ...
                        if (temp.length > 1) {
                            simpleName = temp[temp.length - 1];
                        }

                        values.put(Downloads.COLUMN_TITLE, simpleName);
                    ...
                    try {
                        ...
                        if (Constants.LOGV) {
                            if (InfoHelpers.isNetworkAvailable(mContext)) {
                                Log.i(TAG, "Get Failed " + mInfo.mId + ", Net Up");
                            } else {
                                Log.i(TAG, "Get Failed " + mInfo.mId + ", Net Down");
                            }
                        }
                        if (!InfoHelpers.isNetworkAvailable(mContext)) {
                            finalStatus = Downloads.STATUS_RUNNING_PAUSED;
                            ...
                            if (Constants.LOGV) {
                                Log.d(TAG, "IOException getting entity for " + mInfo.mUri + " : " + ex);
                            }
                            finalStatus = Downloads.STATUS_HTTP_DATA_ERROR;
                        }
                        request.abort();
                    ...
                    for (; ; ) {
                        ...
                        try {
                            ...
                            if (!mInfo.mNoIntegrity && headerETag == null) {
                                if (Constants.LOGV) {
                                    Log.v(TAG, "download IOException for " + mInfo.mUri + " : " + ex);
                                }
                                finalStatus = Downloads.STATUS_PRECONDITION_FAILED;
                            } else if (!InfoHelpers.isNetworkAvailable(mContext)) {
                                finalStatus = Downloads.STATUS_RUNNING_PAUSED;
                            } else if (mInfo.mNumFailed < Constants.MAX_RETRIES) {
                                finalStatus = Downloads.STATUS_RUNNING_PAUSED;
                                countRetry = true;
                        ...
                        if (bytesRead == -1) { // success
                            ...
                            if ((headerContentLength != null) && (bytesSoFar != Integer.parseInt(headerContentLength))) {
                                if (!mInfo.mNoIntegrity && headerETag == null) {
                                    if (Constants.LOGV) {
                                        Log.d(TAG, "mismatched content length " + mInfo.mUri);
                                    }
                                    finalStatus = Downloads.STATUS_LENGTH_REQUIRED;
                                } else if (!InfoHelpers.isNetworkAvailable(mContext)) {
                                    finalStatus = Downloads.STATUS_RUNNING_PAUSED;
                                    ...
                                    if (Constants.LOGV) {
                                        Log.v(TAG, "closed socket for " + mInfo.mUri);
                                    }
                                    finalStatus = Downloads.STATUS_HTTP_DATA_ERROR;
                                }
                                break http_request_loop;
                            ...
                            try {
                                ...
                                if (mInfo.mDestination == Downloads.DESTINATION_EXTERNAL
                                    ...
                                    try {
                                        ...
                                        if (Constants.LOGV) {
                                            Log.v(TAG, "exception when closing the file " + "during download : " + ex);
                                        }
                                        // nothing can really be done if the file can't be closed
                                    }
                                }
                                ...
                                if (!InfoHelpers.discardPurgeableFiles(mContext, Constants.BUFFER_SIZE)) {
                                    finalStatus = Downloads.STATUS_FILE_ERROR;
                                    break http_request_loop;
                                }
                            }
            ...
            if (Constants.LOGV) {
                Log.d(TAG, "Exception for " + mInfo.mUri, ex);
            }
...
//            }
            ...
            if (filename != null) {
                ...
                if (Downloads.isStatusError(finalStatus)) {
                    ...
                    // make sure the file is readable Sync to storage after completion
                    try {
    ...
    private void notifyDownloadCompleted(int status, boolean countRetry, int retryAfter, int redirectCount, boolean gotData, String filename,
        ...
        if (Downloads.isStatusCompleted(status) && Downloads.isStatusSuccess(status) && Constants.MIMETYPE_APK.equals(mimeType)
                && !checkApk(mContext, filename)) {
            status = Downloads.STATUS_FILE_ERROR;
            new File(filename).delete();
        }
        ...
        if (Downloads.isStatusCompleted(status) && Downloads.isStatusSuccess(status)) {

            if (!Constants.MIMETYPE_APK.equals(mimeType)) {
                return;
            }

            try {
                mContext.startActivity(DownloadInstallerActivity.getIntent(mContext, filename));
    ...
    private void notifyThroughDatabase(int status, boolean countRetry, int retryAfter, int redirectCount, boolean gotData, String filename,
                                       String uri, String mimeType) {
        ContentValues values = new ContentValues();
        ...
        mContext.getContentResolver().update(ContentUris.withAppendedId(Downloads.CONTENT_URI, mInfo.mId), values, null, null);
    }

    /**
     * Notifies the initiating app if it requested it. That way, it can know that the download
     * completed even if it's not actively watching the cursor.
     */
    ...
    public boolean checkApk(Context context, String filePath) {
